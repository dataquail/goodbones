...
import { manifestJsonSchema } from "@goodbones/core";
import { Ajv2020 } from "ajv/dist/2020.js";
import { describe, expect, it } from "vitest";
import { parse } from "yaml";

import { CampaignsManifest } from "./spec.js";

// The published manifest schema is the *composed* one: the core's keys and
// the keys of every family a host loads. Both hosts load this family, so the
// file an editor fetches describes `campaigns` and `ledger` too — and this is
// the test that says when it has fallen behind either codec.
//
// It lives here rather than in the core because only a package that can see
// both codecs can generate the composed document. The core's own test covers
// the core's half and the schema mechanics; this one covers the composition,
// the campaign examples from the family's docs page, and the shapes the
// schema must refuse.

const root = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "../../../..");
const committed = path.join(root, "packages/core/schema/architecture.schema.json");

const composedJsonSchema = () =>
  manifestJsonSchema({ extensions: [CampaignsManifest.fields] });

const validator = () => {
  const ajv = new Ajv2020({ allErrors: true, strict: false });
  return ajv.compile(composedJsonSchema());
};

describe("the composed manifest JSON Schema", () => {
  ...
  it("is what packages/core/schema/architecture.schema.json holds", () => {
    const expected = `${JSON.stringify(composedJsonSchema(), null, 2)}\n`;
    expect(readFileSync(committed, "utf8")).toBe(expected);
  });

  it("validates this repository's own manifest, campaigns and all", () => {
  ...
  it("names the recursive detector beside the core's recursive node", () => {
    const schema = composedJsonSchema() as { $defs: Record<string, unknown> };
