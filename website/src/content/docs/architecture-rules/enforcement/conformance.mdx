  ...
  "campaigns": [
    {
      ...
      "fixed": 0,
      "progress": 0,
      "lastProgress": "2026-09-15T21:56:54.278Z",
      "regressions": 0,
      "stalled": false,
      "complete": false,
      "onComplete": "keep",
...
- `campaigns` — every [campaign](/goodbones/architecture-rules/manifest/campaigns/) the
  manifest declares, with its [ledger](/goodbones/architecture-rules/enforcement/ledger/)'s
  burn-down: `initial`, `allowed`, `count`, `fixed`, `progress`
  (`1 − count / (initial + allowed)`), `lastProgress`, how many `regressions` were
  recorded, and whether it is `stalled` (entries left, and `lastProgress` older than its
  `staleAfter`) or `complete`. A campaign with no ledger yet says `ledgered: false`. The
  text report leads its campaigns section with the stalled and the complete ones — a
  migration nobody is working on, and one that is finished and still declared, are the two
  things a reader of this report most needs told.
