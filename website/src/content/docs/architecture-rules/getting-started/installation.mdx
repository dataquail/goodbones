...
Both pull in `@goodbones/core` (the manifest schema and the evaluators),
`@goodbones/typescript` (the TypeScript language pack) and `@goodbones/campaigns` (the
[campaigns family](/goodbones/campaigns/getting-started/introduction/), which the core does not
own) transitively; you never install those directly until a second language pack exists. The pack parses files with
[`oxc-parser`](https://www.npmjs.com/package/oxc-parser) and resolves modules with
[`unrs-resolver`](https://www.npmjs.com/package/unrs-resolver); each ships its own native
...
Or start from the tree instead of a blank page: `architecture infer` describes what the
repository does today as a manifest `check` already accepts, and the work is deleting from it.
See [Starting from the tree](/goodbones/architecture-rules/getting-started/infer/).
